import {
  ...
  XemThongTinApi,
} from "../../api/SinhVienApi";
import {
  CHINHSUATHONGTINSINHVIEN_FAILED,
  CHINHSUATHONGTINSINHVIEN_SUCCESS,
  GETDIEMSINHVIEN_FAILED,
  GETDIEMSINHVIEN_SUCCESS,
  GETDONXINNGHIHOC_OFSINHVIEN_FAILED,
  GETDONXINNGHIHOC_OFSINHVIEN_SUCCESS,
  GETLOPHOCPHANSINHVIEN_FAILED,
  GETLOPHOCPHANSINHVIEN_SUCCESS,
  GETTHONGBAOSINHVIEN_FAILED,
  GETTHONGBAOSINHVIEN_SUCCESS,
  XEMTHONGTINSINHVIEN_FAILED,
...
export const atcGetThongBao = (id, pageNumber) => {
  return (dispatch) => {
    ...
    getThongBaoSinhVienApi(id, pageNumber)
      .then((res) => {
        // const check = localStorage.getItem("thongbao");
        // if (!check)
        // localStorage.setItem(
        //   "thongbao",
        //   res?.data?.thongBaoSinhVienDtos.length
        // );
      ...
      .catch((err) => {
        ...
        dispatch(
          displayNotify({
            message: "Lấy dữ liệu thất bại! Xin load lại trang!",
            type: "warning",
          })
        );
      });
...
export const atcSinhVienXinNghiHoc = (data) => {
  return (dispatch) => {
    ...
    postSinhVienXinNghiHocApi(data)
      .then((res) => {
        dispatch(
          displayNotify({ message: "Gửi thành công!", type: "success" })
        );
      ...
      .catch((err) => {
        ...
        dispatch(hideLoading());
        dispatch(
          displayNotify({ message: "Xin không thành công!", type: "warning" })
        );
      });
  };
};
